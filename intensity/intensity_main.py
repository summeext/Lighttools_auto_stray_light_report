"""
auto_stray_light — Stray Light Auto Analysis (加密环境适配版)
...
def write_macros(project_dir, output_dir):
    ...
    (project_dir / "intensityChart.1.ltb").write_text(
        'REM intensityChart.1.ltb — auto-generated\n'
        'LTBSET UPDATE 0\n'
        'LTBSET DIALOG 0\n'
...
def main(project_dir=None, receiver_name="Receiver_38", b_percent=0.005, max_paths=50):
    ...
    for i in range(1, npaths + 1):
        ...
        path_data.append({
            "path_number": i,
            "max_intensity": float(max_val) if max_val else 0.0,
            "path_detail": pstr,
        })
        print(f"  Path {i}: {path_data[-1]['max_intensity']:.6f}")

    # ---- Step 4: Sort descending ----
    path_data.sort(key=lambda p: -p["max_intensity"])
    top = path_data[0]["max_intensity"]
    threshold = top * (b_percent / 100.0)
    print(f"[Step 4] Max: {top:.8f}, Threshold: {threshold:.8f}")

    # ---- Step 5: Export review.csv ----
    filtered = [p for p in path_data if p["max_intensity"] >= threshold]
    ...
    with open(output_dir / "review.csv", "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["Path Number", "Max Intensity"])
        for p in filtered:
            w.writerow([p["path_number"], f"{p['max_intensity']:.8f}"])
    ...
    for p in filtered:
        ...
        with open(fname, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow(["Path Number", "Max Intensity", "Path Detail"])
            w.writerow([p["path_number"], f"{p['max_intensity']:.8f}", p["path_detail"]])
    print(f"[Step 6] {len(filtered)} path detail CSVs saved")

    # ---- Step 7: Save Forward Intensity chart images (BMP format) ----
    for p in filtered:
        # Isolate path (Python)
        lt.Cmd(f'\\O"{fso_path}"')
        lt.Cmd("HideAll=")
        ...
        s7_imesh_list, _ = t(lt.DbList(s7_fsf_key, "INTENSITY_MESH"))
        ...
        lt.Cmd(f"\\VChart_{receiver_name}_Forward_Intensity")
        time.sleep(0.5)
    ...
    # Main ray path = first in sorted order (highest intensity)
    ...
    main_intensity = main_path["max_intensity"]
    ws_review.append(["Path Number", "Max Intensity", "Intensity Ratio", "Note"])
    for cell in ws_review[1]:
        cell.font = header_font
        cell.fill = header_fill
        cell.border = thin_border
    for p in filtered:
        ratio = p["max_intensity"] / main_intensity if main_intensity else 0
        note = "Main Ray Path" if p is main_path else ""
        ws_review.append([p["path_number"], p["max_intensity"], ratio, note])
    ...
    for p in filtered:
        ...
        c = ws["A1"]
        c.value = f"Path {p['path_number']}  |  Max Intensity: {p['max_intensity']:.8f}  |  Intensity Ratio: {ratio:.2%}"
        if is_main:
    ...
    # Save
    xlsx_path = output_dir / "stray_light_report_intensity.xlsx"
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("Stray Light Auto Analysis (Intensity)")
